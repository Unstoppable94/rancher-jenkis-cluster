...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
  ...
  -Djenkins.install.UpgradeWizard.show=false,jenkins.install.runSetupWizard=false "${jenkins_opts_array[@]}" "$@"
fi
#JAVA_OPTS=-Djenkins.install.runSetupWizard=false
# As argument is not jenkins, assume user want to run his own process, for example a `bash` shell to explore this image
exec "$@"  -Djenkins.install.UpgradeWizard.show=false,jenkins.install.runSetupWizard=false